...
class Program
{
    ...
    {
        // 定义命令行参数
        var rootCommand = new RootCommand("Batch Encoding Converter");
        ...
        rootCommand.Description = "A tool to batch convert file encodings.";

        // 定义命令执行逻辑
        rootCommand.Handler = CommandHandler.Create<string, string, string, string>(ConvertEncoding);

        // 运行命令
        return await rootCommand.InvokeAsync(args);
    }

    private static void ConvertEncoding(string source, string target, string sourceEncoding, string targetEncoding)
    {
        try
        {
            // 获取编码
